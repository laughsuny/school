...
<%@ page import="com.paxonpilot.util.ImageMark"%>
<%@page import="com.paxonpilot.constant.ConfigHolder" %>
<%
...
//水印文件路径 
String iconPath = pageContext.getServletContext().getRealPath("/") + "front/images/icon.png";
	...
	if (!item.isFormField()) {
		...
		if(item.getSize() > maxSize){
			...
			if(maxSizeStr != null || maxSizeStr.length() >0){
				...
				out.println(getError("上传文件大小超过"+sizeM+"M限制。"));
			}else{
				out.println(getError("上传文件大小超过50M限制。"));
			}
			return;
		}
		//检查扩展名
